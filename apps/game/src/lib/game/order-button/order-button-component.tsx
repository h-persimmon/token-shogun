...
import { OrderListener, GameStatusInfo } from '../order-listner';
...
export const OrderButtonComponent: React.FC<OrderButtonProps> = ({ orderListener }) => {
  ...
  useEffect(() => {
    // 即時に最初の状態を取得
    const status = orderListener.getGameStatusInfo();
    setGameStatus(status);
    
    // 選択肢がない場合はデフォルト値をセット
    if (entityId === '' && status.aliveUnitIds.length > 0) {
      setEntityId(status.aliveUnitIds[0]);
    }
    if (targetEnemyTypeId === '' && status.aliveEnemyTypes.length > 0) {
      setTargetEnemyTypeId(status.aliveEnemyTypes[0]);
    }
    if (targetStructureId === '' && status.deployableStructureIds.length > 0) {
      setTargetStructureId(status.deployableStructureIds[0]);
    }
    
    const intervalId = setInterval(() => {
      const newStatus = orderListener.getGameStatusInfo();
  ...
  }, [orderListener, entityId, targetEnemyTypeId, targetStructureId]);
