...
import { createTargetComponent, setEntityTarget } from "../../components/target-component";
...
const createMockEntityManager = () => {
  ...
  return {
    entities,
    createEntity: vi.fn((textureKey: string, x: number, y: number, scale: number) => {
    ...
    queryEntities: vi.fn((query: any) => {
      ...
      for (const entity of entities.values()) {
        const hasRequired = query.required?.every((type: string) => 
...
describe("AttackSystem", () => {
  ...
  describe("setGameStateSystem", () => {
    it("should set GameStateSystem reference", () => {
      ...
      expect(() => attackSystem.setGameStateSystem(newGameStateSystem)).not.toThrow();
    });
  });
  ...
  describe("calculateAndApplyDamage", () => {
    it("should calculate and apply damage correctly", () => {
      ...
      const damageDealt = attackSystem.calculateAndApplyDamage(attackerEntity, targetEntity);
  ...
  describe("applyDamage", () => {
    ...
    it("should notify GameStateSystem when enemy is defeated", () => {
      const targetEntity = createEntity("target", null);
      const healthComponent = createHealthComponent(50);
      const enemyComponent = { type: "enemy" as const, enemyType: "basic" as const, spawnTime: 0, rewardValue: 10, structureTargetPriority: "any" as const  };
      targetEntity.components.set("health", healthComponent);
      targetEntity.components.set("enemy", enemyComponent);

      attackSystem.applyDamage(targetEntity, 50);

      expect(mockGameStateSystem.notifyEnemyDefeated).toHaveBeenCalledWith(targetEntity.id, 10);
    });

    it("should notify GameStateSystem when gate structure is damaged", () => {
      const targetEntity = createEntity("target", null);
      const healthComponent = createHealthComponent(100);
      const structureComponent = { type: "structure", structureType: "gate", isCriticalForLose: true, attackableType: "none", maxUnits: 0 } as const;
      targetEntity.components.set("health", healthComponent);
      targetEntity.components.set("structure", structureComponent);

      attackSystem.applyDamage(targetEntity, 30);

      expect(mockGameStateSystem.notifyStructureDamaged).toHaveBeenCalledWith(targetEntity.id, 30);
    });
  });
  ...
  describe("executeAttack", () => {
    it("should execute direct attack successfully", () => {
      ...
      const result = attackSystem.executeAttack(attackerEntity, targetEntity, currentTime);
    ...
    it("should execute projectile attack successfully", () => {
      const attackerEntity = createEntity("attacker", null);
      const attackComponent = createAttackComponent(50, 200, 1.0, "homing", 300);
    ...
    it("should return failure when attacker has no attack component", () => {
      ...
      const result = attackSystem.executeAttack(attackerEntity, targetEntity, 1000);
